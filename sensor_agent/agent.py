...
from sensor_agent import memory_store
from sensor_agent import trend
...
CHECKPOINT_DB = Path(__file__).parent / "database/agent_checkpoints.db"
...
# ---------------------------------------------------------------------------
# Stub client used ONLY by `langgraph dev` / Studio. Studio imports this
# module looking for a module-level compiled graph -- it never calls main(),
# so there's no real mqtt.Client() connected to a broker at that point. This
# stub gives actuate() something with a .publish() method to call so you can
# simulate nodes in Studio without a live MQTT connection; it just logs.
# ---------------------------------------------------------------------------
class StubMQTTClient:
    def publish(self, topic: str, payload) -> None:
        print(f"[STUB MQTT] would publish to '{topic}': {payload}")
...
# ---------------------------------------------------------------------------
def build_graph(mqtt_client: mqtt.Client, own_checkpointer: bool = True):
...
graph = build_graph(StubMQTTClient(), own_checkpointer=False)
...
def main() -> None:
    ...
    with open(Path(__file__).parent / "images/graph.png", "wb") as f:
